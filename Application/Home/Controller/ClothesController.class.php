...
//衣服首页
    public function index(){
    	...
    	//广告
		$advModel=M('adv');
		$assign['focus']=$advModel->where('type_id=6 and is_show=1')->order('advorder DESC')->limit(3)->select();//衣服首页_广告幻灯三张
		$assign['newPic']=$advModel->where('type_id=7 and is_show=1')->order('advorder DESC')->limit(1)->find();//衣服首页_幻灯右侧一张
		...
		$cat_id = I('get.cat_id',1);//如果cat_id不存在~返回衣服的cat_id=1
		...
		//衣服推荐
		$subIds2 = $categoryModel->getSubIds(1);//获取衣服子类
		...
		/****子分类调用****/
		//衣服————上衣TOPS分类、裤子TROUSERS、裙子DRESSES分类、潮流风格、适用场景、流行元素
		$assign['subCat1']=$catModel->where("parent_id=5 AND is_show=1")->order('sort_order desc,cat_id ASC')->limit(10)->select();
		$assign['subCat2']=$catModel->where("parent_id=6 AND is_show=1")->order('sort_order desc,cat_id ASC')->limit(8)->select();
		$assign['subCat3']=$catModel->where("parent_id=7 AND is_show=1")->order('sort_order desc,cat_id ASC')->limit(11)->select();
		$attr_value=$attrModel->where("attr_id=2")->find();
		$assign['subCat4']=explode(PHP_EOL, $attr_value['attr_value']);
		$attr_value=$attrModel->where("attr_id=1")->find();
		$assign['subCat5']=explode(PHP_EOL, $attr_value['attr_value']);
		$attr_value=$attrModel->where("attr_id=3")->find();
		...
		->where("if43_pro.cat_id in ($subIds2) AND is_onsale=1 AND is_examine=1 AND is_new=1")->order('pro_order DESC,pro_id DESC')->limit(18)->select();
		//小编精选衣服推荐
    ...
    public function prolist() {
    	...
    	//广告
    	$advModel=M('adv');
    	$assign['newPic']=$advModel->where('type_id=8 and is_show=1')->order('advorder DESC')->limit(1)->find();//衣服列表页_精选主题推荐1张
    	$assign['catBg']=$advModel->where('type_id=9 and is_show=1')->order('advorder DESC')->limit(1)->find();//衣服列表页_分类背景1张
    	...
    	//衣服推荐
    	$subIds2 = $categoryModel->getSubIds(1);//获取子类
    	$subIds2=implode(',', $subIds2);
    	$assign['tj1']=$catModel->where("cat_id in ($subIds2) AND is_tj=1 AND is_show=1")->order('sort_order desc')->limit(8)->select();
    	
    	/****子分类调用****/
    	//衣服————上衣TOPS分类、裤子TROUSERS、裙子DRESSES分类、潮流风格、适用场景、流行元素
    	$assign['subCat1']=$catModel->where("parent_id=5 AND is_show=1")->order('sort_order desc,cat_id ASC')->limit(14)->select();
    	$assign['subCat2']=$catModel->where("parent_id=6 AND is_show=1")->order('sort_order desc,cat_id ASC')->limit(7)->select();
    	$assign['subCat3']=$catModel->where("parent_id=7 AND is_show=1")->order('sort_order desc,cat_id ASC')->limit(7)->select();
    	$attr_value=$attrModel->where("attr_id=2")->find();
    	$assign['subCat4']=explode(PHP_EOL, $attr_value['attr_value']);
    	$attr_value=$attrModel->where("attr_id=1")->find();
    	$assign['subCat5']=explode(PHP_EOL, $attr_value['attr_value']);
    	...
    	$assign['top']=$top;
    	if($top=='hot'){
    		$topwhere="AND is_hot=1";
    	}elseif ($top=='new'){
    		$topwhere="AND is_new=1";
    	}else{
    		$topwhere='';
    	}
    	//排序$priceTop条件生成
    	...
    	$assign['maxPrice']=$maxPrice;
    	if($priceTop=='100'){
    		$priceTopsql="AND pro_price<100";
    	}elseif ($priceTop=='200'){
    		$priceTopsql="AND pro_price<200";
    	}elseif ($priceTop=='300'){
    		$priceTopsql="AND pro_price<300";
    	}elseif ($priceTop=='300x'){
    		$priceTopsql="AND pro_price>300";
    	}elseif($minPrice!='' && $maxPrice!=''){
    		$priceTopsql="AND pro_price>$minPrice AND pro_price<$maxPrice";
    	}
    	else{
    		$priceTopsql='';
    	}
    	//排序筛选
    	if($priceTop!=''){
    		$order='pro_price desc,pro_order desc,pro_id desc';
    	}elseif($priceTop=='all' && $top=='new'){
    		$order='add_time desc,pro_order desc,pro_id desc';
    	}
    	else{
    		$order='pro_order desc,pro_id desc';
    	}
    	$proModel=M('pro');
    	$pagesize = 15;
    	$total = $proModel->where("cat_id in ($subIds) AND is_onsale=1 AND is_examine=1 $topwhere $priceTopsql")->count();
    	$opage = new \Think\Page($total, $pagesize);
    	$opage->setConfig('header', '<p>共%TOTAL_ROW%条，共%TOTAL_PAGE%页 当前第%NOW_PAGE%页</p>');
    	$opage->setConfig('prev', '上一页');
    	$opage->setConfig('first', '首页');
    	...
    	$opage->setConfig('theme','%HEADER% %FIRST% %UP_PAGE%  %LINK_PAGE%  %DOWN_PAGE% %END%');
    	$assign['spage'] = $opage->show();
    	$assign['proList'] = $proModel->where("cat_id in ($subIds) AND is_onsale=1 AND is_examine=1 $topwhere $priceTopsql") -> order($order) -> limit($opage->firstRow.','.$opage->listRows)->select();
    	/***********精品列表*************/
    	/*****评价调用*****/
    	$commentModel = M('comment');
    	$assign['commentRows']=$commentModel->join('if43_member ON if43_comment.uid=if43_member.id')
