            ...
            <ul class="sidebar-list">
                ...
                <li>
                    ...
                    <ul class="sub-menu"  style="display:none;">
                        <?php $__FOR_START_22534__=1;$__FOR_END_22534__=70;for($i=$__FOR_START_22534__;$i < $__FOR_END_22534__;$i+=1){ ?><li><a href="system.html"><i class="icon-font">&#xe0<?php echo ($i); ?>;</i><?php echo ($i); ?></a></li><?php } ?>
                    </ul>
                </li>
            </ul>
...
<div class="crumb-wrap">
            <div class="crumb-list"><i class="icon-font"></i><a href="/jscss/admin/design/">首页</a><span class="crumb-step">&gt;</span><a class="crumb-name" href="/Admin/Category/index">精品分类</a><span class="crumb-step">&gt;</span><span>修改分类</span></div>
        </div>
        <div class="result-wrap">
            <div class="result-content">
                <form action="/Admin/Category/edit/cat_id/44" method="post" id="myform" name="myform" enctype="multipart/form-data">
                    <table class="insert-tab" width="100%">
                        <tbody>
                         <tr>
                                <th><i class="require-red">*</i>分类名称：</th>
                                <td>
                                    <input class="common-text required" id="title" name="cat_name" size="50" value="<?php echo ($row["cat_name"]); ?>" type="text">
                                </td>
                            </tr>
                        <tr>
                            ...
                            <td>
                                <select name="parent_id" id="catid" class="required">
                                    ...
                                    <?php if(is_array($cats)): $i = 0; $__LIST__ = $cats;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo['cat_id']); ?>"
                                         <?php if(in_array($vo['parent_id'],$subIds)): ?>disabled='disabled'<?php else: endif; ?>
                                         <?php if(in_array($vo['cat_id'],$subIds)): ?>disabled='disabled'<?php else: endif; ?>
                                         <?php if($vo['cat_id'] == $row['parent_id'] ): ?>selected='selected' <?php else: endif; ?> >
                            ...
                            <tr>
                                <th>是否显示：</th>
                                <td><label><input type="radio" name="is_show" value="1" <?php if($row['is_show'] == 1): ?>checked='checked'<?php else: endif; ?> > 是</label>&nbsp;&nbsp;&nbsp;&nbsp;
                                <label><input type="radio" name="is_show" value="0" <?php if($row['is_show'] == 0): ?>checked='checked'<?php else: endif; ?>> 否</label>
                                </td>
                            </tr>
                            <tr>
                                <th>是否推荐：</th>
                                <td><label><input type="radio" name="is_tj" value="1" <?php if($row['is_tj'] == 1): ?>checked='checked'<?php else: endif; ?> > 是</label>&nbsp;&nbsp;&nbsp;&nbsp;
                                <label><input type="radio" name="is_tj" value="0" <?php if($row['is_tj'] == 0): ?>checked='checked'<?php else: endif; ?>> 否</label>
                                </td>
                            </tr>
                            <tr>
                                <th>内容：</th>
                                <td><textarea name="cat_desc" class="common-textarea" id="content" cols="30" style="width: 98%;" rows="10"><?php echo ($row["cat_desc"]); ?></textarea></td>
                            </tr>
                            <!--<tr>
                            <th></th>
                            <td>
                            ...
                            <tr>
                                <th>排序：</th>
                                <td>
                                    <input class="common-text required" name="sort_order" size="10" value="<?php echo ($row["sort_order"]); ?>" type="text">
                                </td>
                            </tr>
                            <tr>
                                <th></th>
...
<!--<script>
	...
	function dodo() {
		...
		// 当前的ID
		var id = this.value;
		var url = '/Admin/Category/sel/id/'+id; // 将所选分类ID 发送到服务器端
